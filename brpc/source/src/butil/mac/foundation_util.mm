...
//#include "butil/mac/mac_logging.h"
...
bool UncachedAmIBundled() {
  ...
  // Yes, this is cheap.
  return [[butil::mac::OuterBundle() bundlePath] hasSuffix:@".app"];
#endif
}
...
bool IsBackgroundOnlyProcess() {
  ...
  NSDictionary* info_dictionary = [butil::mac::MainBundle() infoDictionary];
...
FilePath PathForFrameworkBundleResource(CFStringRef resourceName) {
  NSBundle* bundle = butil::mac::FrameworkBundle();
...
std::string GetValueFromDictionaryErrorMessage(
  ...
  return "Expected value for key " +
      butil::SysCFStringRefToUTF8(key) +
      " to be " +
      expected_type +
      " but it was " +
      butil::SysCFStringRefToUTF8(actual_type_ref) +
      " instead";
}
...
std::ostream& operator<<(std::ostream& o, const CFStringRef string) {
  return o << butil::SysCFStringRefToUTF8(string);
}
